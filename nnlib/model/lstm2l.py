...
def build(options):
	...
	proj = unit.lstm.build(tparams, 'lstm1', emb, mask, options['dim_wemb'], options['dim_hidden'])
	proj = unit.lstm.build(tparams, 'lstm2', proj, mask, options['dim_hidden'], options['dim_hidden'])
	proj = unit.rnn.postprocess_last(proj)
	...
	if options['decay_c'] > 0.:
		...
		weight_decay = 0.
		weight_decay += (tparams['softmax_W'] ** 2).sum()
