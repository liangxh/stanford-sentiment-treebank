...
def main():	
	...
	optparser.add_option('-i', '--input', action='store', type = 'str', dest='fname_input')
	...
	def build_vocab(seqs):
		...
		total = sum(df.values())
		df = sorted(df.items(), key = lambda k: -k[1])
		...
		for i, item in enumerate(df):
			t, f = item
			last += f / total
			if (not thr_found) and (last >= 0.9):
				df_thr = f
				thr_found = True
			elif thr_found and f < df_thr:
				n_vocab = i
				break
		
		vocabs = [t for t, f in df[:n_vocab]]
		return vocabs

	vocabs = build_vocab(train[0])
	...
	def word_embed(xy, Widx):
		seqs, y = xy
		x = [[Widx[t] for t in toks if Widx.has_key(t)] for toks in seqs]
	
		return (x, y)


	Widx = toWidx(vocabs)
	train = word_embed(train, Widx)
	valid = word_embed(valid, Widx)
	test = word_embed(test, Widx)

	dataset = (train, valid, test)

	def build_Wemb(n_word, dim):
		return 0.01 * np.random.rand(n_word, dim).astype(theano.config.floatX)

	Wemb = build_Wemb(len(vocabs), opts.dim_proj)
