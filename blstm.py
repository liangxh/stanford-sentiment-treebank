...
class Classifier:
	...
	def init_params(self, options, Wemb = None):
		...
		add_params(nnfactory.brnn.init_param(nnfactory.lstm, 'brnn', options['dim_wemb'], options['dim_hidden']))
	...
	def build_model(self, tparams, options):
		...
		proj = nnfactory.brnn.build_layer(nnfactory.lstm, tparams, 'brnn', emb, mask)
		proj = nnfactory.rnn.postprocess_avg(proj, mask)
